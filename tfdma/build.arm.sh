...
CURDIR=$(pwd)
rm -f dmaserver.so
gcc -v -shared  -I"$JAVA_HOME/include" -I"$JAVA_HOME/include/linux" -odmaserver.so -fpic $GITDIR/tfdma/src/main/cpp/com_pointr_tensorflow_api_PcieDMAServer.c
gcc -v -shared  -I"$JAVA_HOME/include" -I"$JAVA_HOME/include/linux" -odmaclient.so -fpic $GITDIR/tfdma/src/main/cpp/com_pointr_tensorflow_api_PcieDMAClient.c
if (( $? != 0 )); then
